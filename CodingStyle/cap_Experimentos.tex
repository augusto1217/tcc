...
Nesta seção são descritas as versões, releases e ferramentas utilizadas no desenvolvimento deste estudo, tais como os sistemas operacionais Windows 8 e Linux Mint 17, o IDE Eclipse, o plugin PyDev, Flake 8, Autopep8 e editor de texto Vim.

\subsection{Sistema Operacional Windows}

Windows 8 é o mais novo membro da família Windows, projetado para ser utilizado em  dispositivos móveis ou computadores pessoais, com uma interface totalmente nova e adaptada para dispositivos sensíveis ao toque \citep{intowindows8-2012}. Neste trabalho foi utilizado o Windows 8.1, versão 6.3.9600, 64 bits, ilustrado na Figura \ref{win8}.

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.32]{figs/win8.png}
\caption{Windows 8 \label{win8}}
\end{figure}

\subsection{Sistema Operacional Linux}
\label{linux_mint}
O Linux Mint é  uma distribuição livre de custos e de código aberto, projetado para ser confortável e fácil de usar, mas também poderoso e configurável sendo voltado para a comunidade de software livre. Baseado em Debian e Ubuntu, é a mais popular distribuição Linux e o terceiro sistema operacional mais utilizado em computadores pessoais, atrás somente de Microsoft Windows e Mac OSX da Apple \citep{linuxmint_2015}. A versão utilizada  neste trabalho foi Linux Mint 17, 64 bits,  ilustrado na Figura \ref{mint}.

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.38]{figs/mint.png}
...
Eclipse Luna Release (4.4.0), build id: 20140612-0600\footnote{\url{http://bit.ly/1EPcAGg}}\footnote{\url{http://bit.ly/1KY7Zqh}}.

A tela principal do Eclipse pode ser vista na Figura  \ref{eclipse}.

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.38]{figs/eclipse.png}
...
PyDev é um plugin do IDE Eclipse, que pode ser usado  para desenvolvimento em Python, Jython e IronPython \citep{pydev_2015}. A Figura \ref{pydev} ilustra o IDE Eclipse com o plugin PyDev. A versão utilizada é 3.9.0\footnote{\url{http://pydev.org/updates}}.

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.38]{figs/pydev.png}
...
É uma ferramenta de verificação de código-fonte em linguagem Python que valida as convenções propostas pelo estilo PEP8 (Anexo \ref{anexoB}). Tem como característica  principal a arquitetura de plugins, que podem ser incorporados  à ferramentas como interpretadores de comando e IDEs. \citep{peptools_2015}
...
É uma ferramenta que permite a verificação de erros de sintaxe, estilo de codificação e padrões em códigos escritos em linguagem Python \citep{flake-8_2015}. A versão utilizada é a 2.2.3 release 25-08-2014, versão baseada em Linux.
...
Uma ferramenta que formata automaticamente o código Python em conformidade com o guia de estilo PEP 8. A ferramenta é capaz de corrigir a maioria dos problemas de formatação que podem ser relatados por pep8. 

A sua instalação se da de acordo com a sintaxe descrita na Tabela \ref{sintxautopep8}

\begin{table}[!ht]
        \caption{Sintaxe de instalação da ferramenta Autopep8.}
        \label{sintxautopep8}
        \centering
        \begin{footnotesize}
        \begin{tabularx}{0.9\textwidth}{>{\ttfamily}X}
        \midrule
        	\textbf{\$} pip install --upgrade autopep8 \\
	\bottomrule
        \end{tabularx}
        \end{footnotesize}
\end{table}

A utilização desta ferramenta é exemplificada na Tabela \ref{exautopep8}

\begin{table}[!ht]
        \caption{Sintaxe de utilização da ferramenta Autopep8.}
        \label{exautopep8}
        \centering
        \begin{footnotesize}
        \begin{tabularx}{0.9\textwidth}{>{\ttfamily}X}
        \midrule
                \textbf{\$} autopep8 --in-place --aggressive --aggressive <filename> \\
        \bottomrule
        \end{tabularx}
        \end{footnotesize}
\end{table}

Na Tabela \ref{fixautoPep8}, são exibidos os códigos de erros que esta ferramenta é capaz de corrigir, seguindo o estilo descrito na seção \ref{pep8}. Para mais informações sobre a 
ferramenta segue o endereço eletrônico \footnote{\url{https://pypi.python.org/pypi/autopep8/}}

\begin{table}[!ht]
        \caption{Códigos de erros que são corrigidos com a ferramenta autopep8.}
        \label{fixautoPep8}
        \centering
        \begin{footnotesize}
        \begin{tabularx}{0.8\textwidth}{>{\ttfamily}lX}
        \toprule
                \textbf{Cód.} & \textbf{Descrição} \\
        \midrule
		E101 & Corrige a indentação de todas as linhas. \\
	\rowcolor[gray]{0.8}
		E121 & Corrige indentação que são múltiplos de quatro. \\
		E122 & Adiciona indentação ausente. \\
	\rowcolor[gray]{0.8}
		E123 & Alinha chaves abertas que coincidem com as chaves fechadas. \\
		E124 & Alinha chaves abertas que coincidem visualmente indentadas. \\
	\rowcolor[gray]{0.8}
		E125 & Corrige a indentação para distinguir linha a partir da próxima linha lógica. \\
		E126 & Corrige indentação pendente. \\
	\rowcolor[gray]{0.8}
		E127 & Corrige não indentado visualmente. \\
		E128 & Corrige não indentado visualmente. \\
	\rowcolor[gray]{0.8}
		E211 & Remover espaços em branco estranhos. \\
		E224 & Remove extraneous whitespace around operator. \\
	\rowcolor[gray]{0.8}
		E226 & Corrige a falta de espaço em branco ao redor de operadores matemáticos. \\
		E227 & Corrige a falta de espaço em branco ao redor de operadores \textit{bitwise/shift}. \\
	\rowcolor[gray]{0.8}
		E228 & Corrige a falta de espaço em branco ao redor do operador módulo. \\
		E231 & Adiciona espaços em branco. \\
	\rowcolor[gray]{0.8}
		E241 & Corrige espaços em branco estranho ao redor de palavras chaves. \\
		E242 & Remove espaços em branco estranhos ao redor de operadores. \\
	\rowcolor[gray]{0.8}
		E251 & Remove espaços em branco ao redor de parâmetro \lq=\rq. \\
		E26  & Corrige espaçamento após comentários com uma única linha. \\
	\rowcolor[gray]{0.8}
		E265 & Corrige espaçamento após bloco de comentário. \\
		E301 & Adicionar linha em branco que está em falta. \\
	\rowcolor[gray]{0.8}
		E302 & Adicionar 2 linhas em branco que está em falta. \\
		E303 & Remove linhas em branco extra. \\
	\rowcolor[gray]{0.8}
		E304 & Remove linhas em branco seguida de funcões. \\
		E309 & Adiciona linha em branco após declaração de classe. \\
	\rowcolor[gray]{0.8}
		E401 & Coloca as importações em linhas separadas. \\
		E501 & Tenta fazer com que a quantidade de caracteres seja igual ao tamanho máximo permitido. \\
	\rowcolor[gray]{0.8}
		E502 & Remove \textit{escape} para nova linha. \\
		E701 & Coloque instrução composta separada por dois pontos em linhas separadas. \\
	\rowcolor[gray]{0.8}
		E711 & Corrigir comparação com Nenhum. \\
		E712 & Corrigir comparação com verdadeiro e falso. \\
	\rowcolor[gray]{0.8}
		E721 & Usa \textit{"isinstance()"} em vez de comparar diretamente os tipos. \\
		W291 & Remove espaço em branco a direita. \\
	\rowcolor[gray]{0.8}
		W293 & Remove espaço em branco a direita em linha em branco. \\
		W391 & Remove linha em branco a direita. \\
	\rowcolor[gray]{0.8}
		W601 & Usa \textit{\lq\lq in\rq\rq} em vez de \textit{\lq\lq hasi\_key()\rq\rq}. \\
		W602 & Corrigir forma obsoleta de elevar exceção. \\
	\rowcolor[gray]{0.8}
		W603 & Usa \lq\lq !=\rq\rq em vez de  \lq\lq <>\rq\rq \\
		W604 & Usa \lq\lq repr()\rq\rq em vez de \textit{backticks}. \\
	\rowcolor[gray]{0.8}
		W690 & Corrigir vários código obsoleto (via lib2to3). \\
        \bottomrule
        \end{tabularx}
        \end{footnotesize}
\end{table}
...
A linguagem de programação Java foi utilizada para criar o algorítimo que captura arquivos compactados e os armazena localmente para a realização da validação deste trabalho.
A versão utilizada para o desenvolvimento dos procedimentos escritos em java é JDK 7 \textit{Java SE Development Kit} release 1.7.0\_75, e pode ser obtido no endereço eletrônico \footnote{\url{http://bit.ly/1mTJaPC}}.

\subsection{Python}

A versão utilizada neste trabalho é a 2.7.9, release 10-12-2014, esta versão faz parte da instalação padrão do sistema operacional citado na seção \ref{linux_mint}, caso deseje executar uma instalação de outra versão superior a utilizada neste estudo, é necessário acessar o sítio oficial da linguagem, na aba de downloads\footnote{\url{https://www.python.org/downloads/}}.

\section{Extrator de métricas}

Neste capítulo detalharemos os procedimentos efetuados para verificar se os projetos contidos  no repositório de software livre PyPI \citep{pypi_2015} escritos em  Python estão aderentes ao padrão descrito na seção \ref{pep8}, e qual o nível de adoção de cada projeto.
...
A primeira etapa consiste na leitura de uma página web do endereço eletrônico\footnote{\url{https://pypi.python.org/pypi?:action=index}} para a obtenção dos endereçamentos dos arquivos compactados. Para a execução desta tarefa foi desenvolvido um procedimento escrito em Java. 
...
\lstinputlisting[language=Java, caption=Código para obter endereços dos pacotes., label=obtLinksPac]{cods/ObterLinkPacotesNew.java}
...
As etapas anteriores foram adotadas a linguagem de programação Java \cite{java_2015} pelo fato de que no início deste trabalho ainda não possuia conhecimento adequado para criar os algoritmos necessários para a obteção dos links e arquivos do repositório do PyPI.

Para a criação e execução dos procedimentos acima mencionados foram encontrados alguns obstáculos a serem superados. A primeira dificuldade foi a de compreender a estrutura do site na qual está localizado o repositório com os arquivos compactados para o download, pois o domínio \citep{pypi_2015} além de conter vários endereços (o que torna a página pesada e extensa de se carregar e navegar), os mesmos não direcionavam ao arquivo desejado e sim a uma nova página que continha uma outra página com uma determinada estrutura que também necessitava de análise para criar rotinas para a localização do arquivo para que pudesse ser salvo no disco rígido.
 
A outra dificuldade foi o tempo necessário para a execução e conclusão do processo de aquisição dos pacotes \citep{pypi_2015} , pois a cada período de execução dos algoritmos o tempo de resposta para a obtenção dos arquivos aumentava. Isto foi consequência do mecanismo de defesa utilizado pelo endereço eletrônico que continha os arquivos compactados, de modo que todo o processo  teve a duração total de 4 dias para finalizar sua execução. A execução do script não aconteceu ininterruptamente pois houveram interrupções e intervenções manuais quando ocorria erros ou tratamento de exceção para os arquivos não encontrados ou endereços eletrônicos inexistentes. 

A Tabela \ref{exctransarq}, exemplifica os tipos de exceções encontradas na transferência dos arquivos.
...
\begin{table}[!ht]
	...
	\begin{footnotesize}
		...
		00:14:12,920 ERROR UrlContent:237 - Software caused connection  \\
		abort: recv failed \\
		00:16:54,874 ERROR UrlContent:237 - Server returned HTTP response \\
		code: 403 for URL: http://jamwt.com/downloads/apricot/apricot-0.2.tar.gz \\
...
\begin{itemize}
	...
	(por exemplo os seguintes caminhos, \lq\lq \texttt{/arquivos-orig/zip}\rq\rq, \lq\lq \texttt{/arquivos-orig/tar.gz}\rq\rq, \lq\lq \texttt{/arquivos-orig/tgz} \rq\rq etc..).
\end{itemize}
