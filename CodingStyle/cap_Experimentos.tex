...
\begin{table}[ht!]
	...
	4006\hspace{3mm} E302 esperado 2 linha em brancos, encontrada 1 \\
	165 \hspace{3mm} E303, muita linha em brancos (4) \\
	325 \hspace{3mm} E401 mútliplosimports on one line \\
	3615\hspace{3mm} E501 linha muito longa (82 characters) \\
	612 \hspace{3mm} W601 .has\_key() is deprecated, usar 'in' \\
	\bottomrule
	\end{tabularx}
\end{table}
...
O desenvolvimento dos scripts se deu pela necessidade de automatizar o processo de identificação e  transferência de um ou mais arquivos de um servidor remoto para um computador local, tornando processo necessários para prosseguir com o procedimento de análise do estilo de codificação recomendadas para a linguagem Python \cite{intoPython-2011} e a quatificação das métricas avaliadas.

\subsection{Rotinas Java}

A primeira etapa se da na leitura de uma página web do endereço eletrônico \footnote{https://pypi.python.org/pypi?:action=index} para a obtenção dos endereçamentos dos arquivos compactados. Para a execução desta tarefa foi desenvolvido um procedimento escrito em Java.
...
Código \ref{extractFile2} - procedimento referenciado no código \ref{extractFile}, que tem como tarefa a extração dos arquivos em um arquivo temporário para sua análise.
...
\begin{itemize}
	...
	(por exemplo, \lq\lq /arquivos-orig/zip\rq\rq, \lq\lq /arquivos-orig/tar.gz\rq\rq, \lq\lq /arquivos-orig/tgz \rq\rq etc..).
\end{itemize}
...
Neste estudo foram removida as verificações para os seguintes códigos de erros: \texttt{E121}; \texttt{E123}; \texttt{E126}; \texttt{E133}; \texttt{E226}; \texttt{E241}; \texttt{E242}; \texttt{E704}. Esta remoção ocorreu pois de acordo com a seção \ref{pep8}, estas regras não são aceitas por unanimidade.

\chapter{Resultados}

Após a execução das rotinas mencionadas nos itens de código \ref{downFile} e \ref{downFile2} obteve-se  um total de \textbf{62.729} arquivos compactados, ocupando \textbf{17.4 Gigabytes} de espaço físico do disco rígido, onde cada arquivo compactado corresponde a um projeto open source escrito em Python. Contudo, foram encontrados \textbf{229} arquivos defeituosos,  os quais foram descartados.
...
Após resultado obtido com a análise descrita na Tabela \ref{exemResult}, ficou constatado que apenas \textbf{3.247} projetos estavam totalmente aderentes ao guia de estilo citado na Seção \ref{pep8}. No Gráfico \ref{allErrors} são exibidos os códigos dos erros que foram encontrados nos projetos analisados.
...
No Gráfico \ref{ge231}, são exibidos os Erro referentes as ocorrências deste erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/E231.png}
\caption{Erro E231. \label{ge231}}
\end{figure}
...
No gráfico \ref{gw191}, são exibidos os Erro referentes as ocorrências deste tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/W191.png}
\caption{Erro W191. \label{gw191}}
\end{figure}
...
No Gráfico \ref{gw293}, são exibidos os Erro referentes as ocorrências deste tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/W293.png}
\caption{Erro W293. \label{gw293}}
\end{figure}
...
Desenvolvedores tem uma dificuldade natural em aderir a este tipo de regra, pois IDEs e editores de texto lhe induziriam a cometer está infração, uma vez que, ao terminar o preechimento de uma linha o editor,  o mesmo posiciona o cursor automaticamente no começo da  nova linha, e não abaixo do comentário.

No Gráfico \ref{ge128}, são exibidos os Erro referentes as ocorrências deste erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/E128.png}
\caption{Erro E128. \label{ge128}}
\end{figure}
...
De acordo com o Anexo A, o erro \texttt{E111} indica que a indentação utilizada não é múltiplo do número 4, ou seja a indentação possui um número ou inferior, ou superior, ou contém espaços em branco.
...
No Gráfico \ref{ge111}, são exibidos os Erro referentes as ocorrências deste tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/E111.png}
\caption{Erro E111. \label{ge111}}
\end{figure}

\section{E265}

De acordo com o Anexo A, o código de erro E265 indica que o bloco de comentário deve iniciar  com jogo da velha (\lq \#\rq) seguido de espaço em branco (\lq \rq).

No Gráfico \ref{ge265}, são exibidos os Erro referente as ocorrências deste tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/E265.png}
\caption{Erro E265. \label{ge265}}
\end{figure}
...
No Gráfico \ref{ge302}, são exibidos os Erro referentes a este tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/E302.png}
\caption{Erro E302. \label{ge302}}
\end{figure}
...
No Gráfico \ref{gw291}, são exibidos os Erro referentes as ocorrências deste tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/W291.png}
\caption{Erro W291. \label{gw291}}
\end{figure}

\section{E251}

De acordo com o Anexo A, o código de erro E251 corresponde a espaços em branco em torno de do caracter \lq\lq =\rq\rq.
...
No Gráfico \ref{ge251}, são exibidos os Erro referentes as ocorrências deste tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/E251.png}
\caption{Erro E251. \label{ge251}}
\end{figure}
...
De acordo com o Anexo A, o código de erro \texttt{E201} corresponde a uma linha de continuação não indentada.

No Gráfico \ref{ge122} são exibidos os Erro referentes a este tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/E122.png}
\caption{Erro E122. \label{ge122}}
\end{figure}
...
No Gráfico \ref{ge201}, são exibidos os Erro referentes as ocorrências deste tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/E201.png}
\caption{Erro E201. \label{ge201}}
\end{figure}
...
\begin{table}[ht!]
        ...
        \begin{tabularx}{0.9\textwidth}{>{\ttfamily}X}
        \midrule
		XML\_FILENAMES = {[} f for f in os.listdir(DIR) \\
                              if f.endswith('.xml') {]} \\
	\bottomrule
        \end{tabularx}
\end{table}
...
No Gráfico \ref{ge203}, são exibidos os Erro referentes a este tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/E203.png}
\caption{Erro E203. \label{ge203}}
\end{figure}
...
\begin{table}[ht!]
		...
		\hspace{20mm}	context1, list(nodeset2.val)), \\
                \hspace{20mm}   ((nodeset2, plF), context1, {[}{]}),{]},\} \\
	\bottomrule
        \end{tabularx}
\end{table}
...
No Gráfico \ref{ge225}, são exibidos os Erro referentes a este tipo de erro.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.65]{figs/E225.png}
\caption{Erro E225. \label{ge225}}
\end{figure}
...
No Gráfico \ref{ge202}, são exibidos os Erro referentes a este tipo de erro.

\begin{figure}[ht]
\centering
\includegraphics[scale=0.95]{figs/E202.png}
\caption{Erro E202. \label{ge202}}
\end{figure}
...
Devido ao fato de dois códigos de erros apresentarem uma quantidade de ocorrências muito elevada e possuirem um baixo grau de complexidade para a sua correção, foi executada uma nova avaliação dos Erro obtidos, excluindo as regras \texttt{E501} e \texttt{E231} e verificando o impacto destas regras no nível  de adoção do estilo de código discutido neste estudo.
...
O total de ocorrências dos códigos de erros analisados é de \textbf{45.356.622}, portanto corresponde a \textbf{87,8723179} porcento dos erros ocorridos em todo o projeto.
