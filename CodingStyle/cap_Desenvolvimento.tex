...
Também é possível que o desenvolvedor faça uma contribuição para o documento que descreve o estilo de codificação GNU/C. Caso ele detecte alguma inconsistência ou uma possível evolução em um determinado conteúdo, basta enviar um email para bug-standards@gnu.org com a descrição de suas contribuições \cite{stallman2015}.
...
As linguagens Java e C possuem outros tipos estruturas comuns entre si, as quais podem ser consultadas no guia de estilo das linguagem citadas\footnote{http://www.gnu.org/prep/standards/}\footnote{http://www.oracle.com/technetwork/java/codeconvtoc-136057.html}.

\subsection{Python}

De acordo com \cite{histpython1990}, a linguagem Python foi criada no início de 1990 por Guido van Rossum no instituto de pesquisa \textit{Stichting Mathematisch Centrum} -- CWI\footnote{http://www.cwi.nl}, localizado na Holanda. Esta linguagem é considerada uma sucessora da linguagem ABC.
...
Todas as versões da linguagem Python são \textit{open source}\footnote{http://opensource.org}. A maioria das versões de Python são compatíveis com a licença GPL; a Tabela \ref{versoesPython} resume as principais características de cada versão.
...
O guia de estilos proposto para a linguagem Python no item citado na Seção \ref{pep8} é extenso, e pode ser consultado em \cite{pep8-2001} e no Anexo \ref{anexoB}.
