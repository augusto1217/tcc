...
Neste capítulo será apresentado o histórico dos estilos de codificação, que tiverem origem com a notação húngara, nos anos 70, passando pelos estilos mais prominentes, como os estilos Gnu/C, Java e Python. Em seguida, será apresentado o estilo PEP8 da linguagem Python, que será objeto deste estudo. Por fim, é descrito brevemente o conceito de ambientes integrados de desenvolvimento -- IDEs.

\section{Histórico dos estilos de codificação}

\subsection{Notação Húngara}

A notação húngara, hoje chamada Apps húngaro, foi inventada por Charles Simonyi, um programador que trabalhou na Xerox PARC entre1972 e 1981, e que mais tarde se tornou arquiteto-chefe da Microsoft \citep{notacao2015}.
 
A notação é uma referência para os estilos de codificação existentes, pois foi a partir dela que teve início o processo de definir um determinado estilo de codificação a ser seguido dentro de um projeto. A notação húngara utiliza prefixos para indicar o tipo de informação que está sendo  armazenada, fazendo com que desenvolvedores decorem nomes de identificadores baseados na informação semântica que eles armazenam \citep{notacao2015}.  Esses prefixos são letras minúsculas associadas ao conteúdo ou tipo da variável ou identificador. A Tabela \ref{tab2} lista os indicadores mais utilizados na notação húngara.
...
\begin{table}[!ht]
        ...
        \centering
	\begin{footnotesize}
        \begin{tabularx}{0.7\textwidth}{>{\ttfamily}lX}
        \toprule
        	\textbf{Nome} & \textbf{Descrição} \\
        \midrule
	       	s & \textit{string} \\ 
        \rowcolor[gray]{0.8}
		sz & Aponta o primeiro caracter de uma \textit{string} terminada em zero \\ 
		st & Ponteiro da \textit{string}, o primeiro \textit{byte} é contador dos caracteres \\ 
        \rowcolor[gray]{0.8}
		h & \textit{Handle} \\ 
		msg & Mensagem \\ 
        \rowcolor[gray]{0.8}
		...
		c & caracterer (8 bits) \\ 
        \rowcolor[gray]{0.8}
		by & unsigned char (\textit{byte} ou caractere não sinalizado de 8 bits) \\ 
		n & Inteiro \\ 
        \rowcolor[gray]{0.8}
...
\begin{table}[!ht]
	...
	\begin{footnotesize}
        ...
        \toprule
                \textbf{Nome} & \textbf{Descrição} \\
        \midrule
                bBusy & Verdadeiro ou Falso \\
	\rowcolor[gray]{0.8}
		chInitial & caracterer \\ 
		cApples & contador de itens \\ 
	\rowcolor[gray]{0.8}
		dwLightYears & \textit{double} palavra \\ 
		fBusy & float \\ 
	\rowcolor[gray]{0.8}
		nSize & Inteiro ou contador \\ 
		iSize & Inteiro ou índice \\ 
	\rowcolor[gray]{0.8}
		fpPrice & ponto flutuante \\ 
		dbPi & \textit{double} \\ 
	\rowcolor[gray]{0.8}
		pFoo & ponteiro \\
		rgStudents  & \textit{array}, ou intervalos de valores \\ 
	\rowcolor[gray]{0.8}
		szLastName  & terminador zero \textit{string} \\
		u32Identifier  & Inteiro não sinalizado de 32-bit \\
	\rowcolor[gray]{0.8}
		stTime & estrutura de tempo \\
		fnFunction  & nome de função \\
	\rowcolor[gray]{0.8}
		pszOwner  & ponteiro para terminador zero \textit{string} \\
		rgfpBalances  & \textit{array} para valores de ponto flutuante \\
	\rowcolor[gray]{0.8}
		aulColors & \textit{array} \textit{long} não sinalizado \\
		hwndFoo  & \textit{Handle} para um \textit{window} \\
	\rowcolor[gray]{0.8}
		lpszBar  & \textit{long} ponteiro \textit{string} para um terminador zero \\
		g\_nWheels & membros de um \textit{namespace} global, inteiro \\
	\rowcolor[gray]{0.8}
		m\_nWheels & membros de uma estrutura/classe, inteiro \\
		m\_wheels,\_wheels  & membros de uma estrutura/classe \\
	\rowcolor[gray]{0.8}
		s\_wheels  & membros estáticos de uma classe \\
		c\_wheels & membros estáticos de uma função \\ 
	\bottomrule
        \end{tabularx}
	\end{footnotesize}
\end{table}

O sistema húngaro foi adotado como padrão de programação do Windows, e se espalhou amplamente por meio de livros como \citep{charles2002}, a bíblia para aprender programação do Windows . Rapidamente a notação húngara se tornou o estilo de codificação dominante utilizado em códigos escritos em linguagem C \citep{notacao2015}.

Mas, por algum motivo não divulgado ao público, com a primeira versão da plataforma .NET, a Microsoft começou a abandonar o uso da notação húngara. A equipe de projetos do Excel e Word ficaram aliviados em não ter que usar uma convenção de nomenclatura estranha, já que nos dias atuais a verificação de declaração dos tipos de variáveis é feita pelo compilador e há o recurso de autocompletar o código digitado que sugere um conjunto de opções ao programador, a depender do contexto \citep{notacao2015}.
...
Para que um novo desenvolvedor possa participar de algum projeto ou criar algo para a comunidade do software livre GNU, é exigido apenas que ele se atente ao  documento oficial de estilos\footnote{\url{http://www.gnu.org/prep/standards/}}.  É de suma importância que se utilize a versão atualizada, para que não seja escrito um código formatado sobre regras que possam ter sido revogadas ou substituídas.

Também é possível que o desenvolvedor faça uma contribuição para o documento que descreve o estilo de codificação GNU/C. Caso ele detecte alguma inconsistência ou uma possível evolução em um determinado conteúdo, basta enviar um email para bug-standards@gnu.org com a descrição de suas contribuições \citep{stallman2015}.
...
O estilo de codificação Gnu/C utiliza 79 caracteres para o comprimento das linhas. Segundo \cite{stallman2015}, esta caraterística facilita a leitura do código nos mais diversos ambientes e editores.
O estilo também recomenda a utilização de chaves \lq\lq \{ \}\rq\rq\ para o bloco da função em linha posterior à da declaração da função e ao final do bloco. O Código \ref{formatGnuC} exemplifica a formatação mencionada.

\lstinputlisting[language=C, caption=Formatação GNU/C, label=formatGnuC]{cods/formatgnu.c}

\subsubsection{Comentário}

\cite{stallman2015} diz que todo programa deve começar com um comentário no topo do código-fonte contendo a função \lq principal\rq\ do programa. Este comentário deve informar o nome do arquivo e apresentar uma ou duas linhas a respeito do objetivo geral do programa. As estruturas e as funções devem trazer comentários com a descrição sobre sua funcionalidade, quais os tipos de argumentos pode receber e o seu tipo de retorno, quando houver.
...
O Código \ref{palchaveGnuC} exemplifica algumas palavras chaves que utilizam espaço após sua declaração.

\lstinputlisting[language=C, caption=Palavras chaves espaços em branco no estilo Gnu/C, label=palchaveGnuC]{cods/palchavegnu.c}

Já o Código \ref{palchaveExcGnuC} exemplifica algumas palavras chaves que não  utilizam espaço após sua declaração.

\lstinputlisting[language=C, caption=Palavras chaves exceção espaços em branco no estilo Gnu/C, label=palchaveExcGnuC]{cods/palchaveexcgnu.c}

\subsection{Java}

Após ter sua solicitação de padronização negada pelos orgãos ISO/IEC e ECMA a empresa \textit{Sun Microsystems} criou, em 1998, o \textit{Java Community Process} (JCP), com o objetivo de ser um processo aberto e participativo para desenvolver e revisar as especificações de suas implementações de referência, das suítes de teste e do estilo de codificação da tecnologia Java.
...
De acordo com \cite{oracle2015}, todos os códigos-fontes devem começar com um comentário semelhante ao estilo GNU/C citado na Seção \ref{sec:formatacao}. No estilo Java é necessário que o comentário contenha o nome da classe, informações sobre a versão, a data de criação e o aviso de direitos autorais.  Os comentários de implementação são delimitados por \texttt{/*...*/} ou precedidos pelos caracteres \texttt{//}. Os comentários de documentação são conhecidos como \lq\lq comentários doc\rq\rq\ e são delimitadas pelos marcadores \texttt{/**} e \texttt{*/}.

O Código \ref{comentjava} exemplifica o estilo de comentário Java para início de uma classe.

\newpage

\lstinputlisting[language=Java, caption=Comentário Java. \citep{oracle2015}\, com adaptações, label=comentjava]{cods/coment.java}
...
A Tabela \ref{tabtagsJavaDoc}, exibe algumas tags utilizadas pelo javaDoc e seu respectivo significado, e o Código \ref{utiltagsjava}, exemplifica a utilização de comentário no padrão javaDoc.
\begin{table}[!ht]
        \caption{Tags JavaDoc.}
        \label{tabtagsJavaDoc}
        \centering
	\begin{footnotesize}
        \begin{tabularx}{0.9\textwidth}{>{\ttfamily}lX}
        \toprule
	...
	\rowcolor[gray]{0.8}
		@see & Possibilita a definição de referências de classes ou métodos, que podem ser consultadas para melhor compreender a idéia daquilo que está sendo comentado.\\
...
\begin{table}[!ht]
        \caption{Tipos de Identificadores Java (\citep{convencoesjava2013}, com adaptações).}
        \label{tabtpidentJava}
        \centering
	\begin{footnotesize}
        \begin{tabularx}{0.99\textwidth}{lXX}
        \toprule
                \textbf{Tipo de Identificador} & \textbf{Regras Para Nomes} & \textbf{Exemplos} \\
        \midrule
		Pacotes & O prefixo do nome do pacote deve ser único, deve sempre ser escrito em letras minúsculas em ASCII e deve ser um dos nomes de domínio de nível superior, tal como 			especificado na norma \citep{iso3166-2015}. Componentes subseqüentes do nome do pacote, varia de acordo com a organização da convenções de nomenclatura. & 
...
\subsubsection{Espaços em branco em expressões}
...
Duas linhas em branco devem ser sempre utilizadas nas seguintes circunstâncias:
...
As linguagens Java e C possuem outros tipos estruturas comuns entre si, as quais podem ser consultadas no guia de estilo das linguagem citadas\footnote{\url{http://www.gnu.org/prep/standards/}}\footnote{\url{http://www.oracle.com/technetwork/java/codeconvtoc-136057.html}}.

\subsection{Python}

De acordo com \cite{histpython1990}, a linguagem Python foi criada no início de 1990 por Guido van Rossum no instituto de pesquisa \textit{Stichting Mathematisch Centrum} -- CWI\footnote{\url{http://www.cwi.nl}}, localizado na Holanda. Esta linguagem é considerada uma sucessora da linguagem ABC.
...
As ferramentas e a documentação da linguagem Python são softwares de código aberto com uma licença compatível com a \textit{General Public License} (GPL), porém menos restritiva, permitindo que o Python seja inclusive incorporado em produtos proprietários. A especificação da linguagem é mantida pela \textit{Python Software Foundation} (PSF). Além de ser utilizada como linguagem principal no desenvolvimento de sistemas, o Python também é muito utilizado como linguagem script em vários softwares, permitindo automatizar tarefas e adicionar novas funcionalidades em softwares desenvolvidos primariamente em outras linguagens \citep{guido2011}.

A implementação oficial do Python é mantida pela PSF e escrita em C e, por isso, é também conhecida como CPython\footnote{\url{http://www.python.org/download/}}. Para a plataforma Windows, basta executar o instalador. Para outras plataformas, como em distribuições Linux, geralmente o Python já faz parte da distribuição; porém em alguns casos pode ser necessário compilar e instalar o interpretador a partir dos arquivos fonte.

Existem também implementações de Python para .NET (IronPython)\footnote{\url{http://ironpython.net/}}, JVM (Jython)\footnote{\url{http://www.jython.org/}} e em Python (PyPy)\footnote{\url{http://pypy.org/}} \citep{guido2011}.

Todas as versões da linguagem Python são \textit{open source}\footnote{\url{http://opensource.org}}. A maioria das versões de Python são compatíveis com a licença GPL; a Tabela \ref{versoesPython} resume as principais características de cada versão.

\begin{table}[!ht]
        \caption{Versões Python (\citep{histpython1990}, com adaptações).}
        \label{versoesPython}
        \centering
        \begin{footnotesize}
        ...
        \midrule
		0.9thru .0 1.2 & n/a & 1991-1995 & CWI & sim \\
	\rowcolor[gray]{0.8}
	...
	\rowcolor[gray]{0.8}
		...
		2.2 e acima & 2.1.3 & 2001- atualmente & PSF & sim \\
	\bottomrule
        \end{tabularx}
        \end{footnotesize}
...
Segundo \cite{googlestyle}, todo grande projeto de código aberto tem seu próprio guia de estilo, que por sua vez possui um conjunto de convenções, por vezes arbitrário, sobre como escrever código para o projeto. 
...
Códigos-fontes escritos em Python deve sempre utilizar codificação ASCII -- \textit{American Standard Code for Information Interchange} -- ou ISO-8859-1, que é uma codificação de caracteres do alfabeto latino. Para as versões Python 2.7 admite UTF-8,para facilitar a transição para versões superiores, já para o Python 3.0 ou superiores é preferível a utilização da codificação UTF-8 -- \textit{8-bit Unicode Transformation Format} \footnote{\url{http://www.python.org/dev/peps/pep-3120}}.
...
O Código \ref{importpy} exemplifica a utilização de importação de pacotes segundo às recomendações citada na Seção \ref{pep8}.

\lstinputlisting[language=Python, caption=Importação de pacotes conforme o estilo Python., label=importpy]{cods/import.py}

O Código \ref{importnotstylepy} exemplifica a utilização de importação de pacotes de forma divergente à proposta no estilo citado na Seção \ref{pep8}.

\lstinputlisting[language=Python, caption=Importação de pacotes que não segue o estilo PEP8., label=importnotstylepy]{cods/importnotestyle.py}
...
Comentários de bloco normalmente consistem em um ou mais parágrafos, divididos em múltiplas linhas; já os comentários em uma mesma linha devem ter ao menos dois espaços de distância do último caractere e começar com um \lq \texttt{\#}\rq, um espaço e o texto.
...
\cite{pep257-2001} descrevem como escrever um comentário em formato docstring, o qual consiste em uma cadeia de caracteres que ocorre como a primeira declaração em um módulo, função, classe ou definição de método. 

O Código \ref{docstringpy} exemplifica um comentário em formato docstrings.
...
De acordo com \cite{pep8-2001}, não se pode misturar tabulações e espaços para a utilização de indentação: o recomendado é que se utilize 4 espaços para a indentação dos blocos. Para projetos mais antigos  a orientaçao é utilizar uma tabulação (tecla \texttt{TAB}), 
...
\item não usar espaços em volta do caractere \lq \texttt{=}\rq\ quando é antecedido por uma palavra-chave de um argumento ou sucedido por um valor padrão para um parâmetro.
\end{enumerate}
...
Segundo \cite{pep8-2001} as convenções de nome da biblioteca padrão da linguagem Python são confusas,
 e inconsistentes. No entanto, são recomendadas as seguintes convenções para a construção de nomes e identificadores:
...
Nunca utilize os caracteres \lq \texttt{l}\rq\ (ele minúsculo), \lq\texttt{O}\rq\ (o maiúsculo), ou \lq\texttt{I}\rq, (i maiúsculo) como nome de uma variável de apenas um caractere pois, em determinadas fontes, esses caracteres são indistinguíveis dos numerais \texttt{1} (um) e \texttt{0} (zero). Quando for necessário usar o \lq\texttt{l}\rq, use o caractere \lq\texttt{L}\rq\ em seu lugar.
...
Em relação aos métodos e classes, utilize sempre o identificador \lq\lq\texttt{self}\rq\rq\ como primeiro argumento de métodos de instância e usar \lq\lq\texttt{cls}\rq\rq\ como primeiro argumento para métodos de classe. 

O guia de estilos proposto para a linguagem Python no item citado na Seção \ref{pep8} é extenso, e pode ser consultado em \citep{pep8-2001} e no Anexo \ref{anexoB}.
...
IDEs \textit{Integrated Development Environment} são pacotes de software que integram várias ferramentas de desenvolvimento em um ambiente consistente, com o objetivo de aumentar a produtividade do desenvolvedor. Geralmente, IDEs incluem recursos como \textit{syntax highlight} (código fonte colorizado conforme a sintaxe da linguagem), navegadores de código, shell integrado e code completion (o editor apresenta, durante a digitação, formas possíveis de completar o texto que ele consegue identificar) \citep{guido2011}.

Entre as IDEs que suportam Python, encontram-se: 

\begin{itemize}
\item Idle\footnote{\url{https://docs.python.org/2/library/idle.html}}
\item PyCharm\footnote{\url{https://www.jetbrains.com/pycharm/}}
\item PyScripter5\footnote{\url{https://code.google.com/p/pyscripter/wiki/FAQ}};
\item SPE6 (Stani's Python Editor)\footnote{\url{http://sourceforge.net/p/spe/wiki/Home/}};
\item Eric7\footnote{\url{http://eric-ide.python-projects.org/}};
