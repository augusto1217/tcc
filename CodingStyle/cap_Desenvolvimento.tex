...
\lstinputlisting[language=C, caption=Palavras chaves espaços em brancos no estilo Gnu/C, label=palchaveGnuC]{cods/palchavegnu.c}

Já o Código \ref{palchaveExcGnuC} exemplifica algumas palavras chaves que não  utilizam espaço após sua declaração.

\lstinputlisting[language=C, caption=Palavras chaves exceção espaços em brancos no estilo Gnu/C, label=palchaveExcGnuC]{cods/palchaveexcgnu.c}
...
Nunca utilize os caracteres \lq \texttt{l}\rq\ (ele minúsculo), \lq\texttt{O}\rq\ (o maiúsculo), ou \lq\texttt{I}\rq, (i maiúsculo) como nome de uma variável de apenas um caractere pois, em determinadas fontes, esses caracteres são indistinguíveis dos numerais \texttt{1} (um) e \texttt{0} (zero). Quando  for necessário a usar o \lq\texttt{l}\rq, usar o caractere \lq\texttt{L}\rq\ em seu lugar.
