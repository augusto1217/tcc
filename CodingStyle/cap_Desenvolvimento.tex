...
\begin{table}[ht]
		...
		c & char (8 bits) \\ 
        \rowcolor[gray]{0.8}
		by & unsigned char (byte ou caractere não sinalizado de 8 bits) \\ 
		n & Inteiro \\ 
        \rowcolor[gray]{0.8}
...
O estilo também recomenda a utilização de chaves "\{ \}" para o bloco da função em linha posterior à da declaração da função e ao final do bloco. O Código \ref{formatGnuC} exemplifica a formatação mencionada.
